...
# Functionality
    def upload(self, filePath):
        ...
        response = self.send_data("/file/upload?"+data, f, headers)
        f.close()
        thread.join()
        if len(response.sMessages) > 0:
            for sMessage in response.sMessages:
    ...
    def share(self, fileIds, targetIds, shareType):
        ...
        for targetId in targetIds:
            headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/xml"}
            if shareType=="user":
                data = urllib.urlencode({'fileIds': fileIds, 'userId': targetId, 'format': 'cli', 'requestOrigin': self.requestOrigin})
                response = self.send_data("/share/create_user_shares", data, headers)
            elif shareType=="group":
                data = urllib.urlencode({'fileIds': fileIds, 'groupId': targetId, 'format': 'cli', 'requestOrigin': self.requestOrigin})
                response = self.send_data("/share/create_group_shares", data, headers)
            else:
                sys.exit(0)
            if len(response.sMessages) > 0:
                for sMessage in response.sMessages:
    ...
    def download(self, fileIds):
        ...
        try:
            ...
            for fileId in fileIds:
                ...
                response = urllib2.urlopen(self.serverLocation + "/file/download", data)
    ...
    def delete(self, fileIds):
        ...
        """
        data = urllib.urlencode({'fileIds': fileIds, 'format': 'cli', 'requestOrigin': self.requestOrigin})
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/xml"}
        response = self.send_data("/file/delete_files", data, headers)
        if len(response.sMessages) > 0:
            for sMessage in response.sMessages:
                self.cli_write(sMessage)
                self.headerMessages.append(sMessage)
    ...
    def show_files(self):
        ...
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/xml"}
        response = self.send_data("/file/get_user_file_list", data, headers)
        if len(response.sMessages) > 0:
            for sMessage in response.sMessages:
                self.cli_write(sMessage)
        if len(response.fMessages) > 0:
    ...
    def show_groups(self):
        ...
        data = urllib.urlencode({'format': 'cli'})
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/xml"}
        response = self.send_data("/account/get_groups", data, headers)
        if len(response.sMessages) > 0:
            for sMessage in response.sMessages:
                self.cli_write(sMessage)
        if len(response.fMessages) > 0:
...
# Helper Functions
    ...
    def poll_upload(self, fileName):
        ...
        ellipsisCount = 3
        response = self.send_data("/file/upload_stats", data, headers)
        if len(response.data) > 0:
